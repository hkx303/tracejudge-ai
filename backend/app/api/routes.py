...
def confirm_analysis(analysis_id: str, request: ConfirmationRequest):
    ...
    case_path.write_text(f"# 已确认案例 {analysis_id}\n\nverified: true\n\n- 归因：{request.classification.value}\n- 根因：{request.actual_root_cause}\n- 确认人：{request.confirmed_by}\n", encoding="utf-8")
    knowledge.reindex()
